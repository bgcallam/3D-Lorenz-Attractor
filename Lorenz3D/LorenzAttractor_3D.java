import processing.core.*; import java.applet.*; import java.awt.*; import java.awt.image.*; import java.awt.event.*; import java.io.*; import java.net.*; import java.text.*; import java.util.*; import java.util.zip.*; public class LorenzAttractor_3D extends PApplet {//import processing.opengl.*;
...
public void setup()
{
  ...
  colorMode(HSB, 100);
  colorMode(RGB);
  perspective( PI*0.45f, width/height, 1.0f, 700 );
  //smooth();
  lights();
...
public void draw()
{
  ...
  camera(elements[elements.length-1].x0*10, elements[elements.length-1].y0*10, (elements[elements.length-1].z0*10)+(height-mouseY)*2, elements[1].x0, elements[1].y0, elements[1].z0, 0.0f, 0.0f, -1.0f);
...
class Lorenz
{
  ...
  static public void main(String args[]) {     PApplet.main(new String[] { "LorenzAttractor_3D" });  }}